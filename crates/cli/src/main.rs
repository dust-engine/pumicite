...
// ---------------------------------------------------------------------------
// slang reflection → PipelineLayout
// ---------------------------------------------------------------------------
...
fn build_pipeline_layout(reflection: &slang::reflection::Shader) -> PipelineLayout {
    ...
    for set_idx in 0..global_type_layout.descriptor_set_count() {
        if let Some(set) = build_descriptor_set(global_type_layout, set_idx, &stages) {
            sets.push(set);
        }
    }
    ...
    for entry_point in reflection.entry_points() {
        ...
        for set_idx in 0..type_layout.descriptor_set_count() {
            if let Some(set) = build_descriptor_set(type_layout, set_idx, ep_stages) {
                sets.push(set);
            }
        }
    }
